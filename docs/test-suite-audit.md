...
- The T-121 Release batch passed 373/374 tests but `ListBoxPartialSelectionPruneScalesLinearly` failed its wall-clock ratio (`9515` versus `65256` ticks for four times the items). Its retained bounded native-query evidence remained correct.

This is a pure test-suite consolidation. No production behavior was changed. The retained suite contains 370 tests, a reduction of 19.
...
| `ListBoxPartialSelectionPruneScalesLinearly`; `ListBoxPartialSelectionPruneReadsItemCountOnce` | Consolidated a scheduler-sensitive wall-clock ratio with its deterministic operation-count proof. | `ListBoxPartialSelectionPruneUsesBoundedNativeQueries` retains partial-prune behavior and directly bounds `LB_GETCOUNT`, bulk selection, and per-item selection messages. |
...
- `ListBoxPartialSelectionPruneScalesLinearly` and `ListBoxPartialSelectionPruneReadsItemCountOnce` became `ListBoxPartialSelectionPruneUsesBoundedNativeQueries`; it retains the partial-prune contract and proves bounded native work without wall-clock timing.
...
- Initial consolidated Debug and two consecutive Release Win32 runs passed 371/371 tests with zero failed, errored, leaked, skipped, or ignored tests.
- After T-121 added three grid regressions, the corrected candidate passed 373/373 in Debug and Release with the same zero counts. The underlying retained baseline is 370 tests.
